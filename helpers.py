...
def getAviData(zone_id: int, date_of_pull: str):
...
# function needed to getAviData_Optimized
def get_report_ID_and_process(report_id, year, url, csv_file):
    # build form_data to submit post
    # this is basically like clicking a button to select a report from the archive
    # VERY SLOW WITHOUT OPTIMIZATION
    form_data = {
            '_qf__arc_form' : '',
            'arc_bc_avo_fx_sel[0]': year,
            'arc_bc_avo_fx_sel[1]': report_id,
    }
    
    # This step is very expensive since I have to make a request to CAIC's site
    response = requests.post(url, data=form_data)

    # Occasionally I would overload CAIC's server and get kicked off, so this bit of code 
    # gives their page a quick break if the page does not load

    # if the request does not yield a 200 status code, wait 10 seconds and try again
    while response.status_code != 200:
        time.sleep(10)
        response = requests.post(url, data=form_data)

    # GATHER DATA through HTML parsing
    # Build this area if you want to retrieve more than just avi danger from forecast
    tree = lh.document_fromstring(response.content)
...
############################## getAviData_Optimized ##############################

# Optimized getAviData by using multithreading
def getAviData_Optimized(zone_id: int, date_of_pull: str):
    url = 'https://avalanche.state.co.us/caic/pub_bc_avo.php?zone_id=' + str(zone_id)
    csv_file = date_of_pull + "_zone_" + str(zone_id) +"_aviDanger" + ".csv"
    with open(csv_file, "a") as file:
        file.write("date,danger_below,danger_near,danger_above" + "\n")
    ...
    # Iterate over dates year 2013 through 2020
    # This can be adjusted if just looking for specific dates. The current data goes back to 2013 though
    years = ['2013', '2014', '2015', '2016', '2017', '2018', '2019', '2020']

    for year in years:
        # initiate array of reports by year
        report_array = reports[year]['values']
        
        for x in range(len(report_array)):
            # Start threading here to try to run get requests in parallel
            # See get_report_ID_and_process for code and usage
            # This is the step you need to optimize for significant performance gain
            _thread.start_new_thread(get_report_ID_and_process, (report_array[x], year, url, csv_file) )  

            
# Function to sort data by date and save to new path
